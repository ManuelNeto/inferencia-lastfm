...
funcao_theta_correlacao = function(df) {
    cor(df$mediana_pop, df$proporcao, method = "pearson")
}

theta_c_correlacao = funcao_theta_correlacao(lastfm)
```
...
reamostragens_correlacao = tibble(i = 1:repeticoes) %>% 
  mutate(theta_c_s_correlacao = map(i, ~ um_bootstrap_correlacao(lastfm))) %>% 
  unnest(theta_c_s_correlacao)

reamostragens_correlacao

reamostragens_correlacao %>%
  ggplot(aes(x = theta_c_s_correlacao)) +
  geom_histogram(binwidth = .5,
                 colour = "darkorange",
                 fill = "white")

reamostragens_correlacao %>%
  ggplot(aes(x = theta_c_s_correlacao - theta_c_correlacao)) +
  geom_histogram(binwidth = 1,
                 colour = "darkblue",
                 fill = "white")
...
```{r}
intervalo = reamostragens_correlacao %>% 
...
```{r}
intervalo = intervalo %>% 
  mutate(inf_value = theta_c_correlacao + inf_error, 
         sup_value = theta_c_correlacao + sup_error)
intervalo
```
...
ggplot() +
  ...
  geom_histogram(
    data = reamostragens_correlacao,
    aes(theta_c_s_correlacao),
    binwidth = .5,
    fill = "white",
    colour = "darkgrey"
  ) +
  geom_vline(xintercept = theta_c_correlacao, color = "dark green") +
  labs(title = expression("Intervalo estimado via bootstrap"))
